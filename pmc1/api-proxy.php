<?php
/**
 * PMC1 Gemini Proxy — Pasco Municipal Code AI Reference
 * Based on coach5/api-proxy.php; adapted for professional municipal code reference use.
 */
set_time_limit(300);
...
//  80,000 chars ≈  20,000 tokens  — allows ~3-4 pages of document paste
...
// engineers hit a warm cache at any point from morning to end of day.
// The underlying Files API URI is separate (48h TTL, managed by cache-create.php + cron).
//
...
function handle_stream($data, $secretsFile, $cacheNameFile) {
    ...
    // code falls through to the Files API path with no user-visible impact.
    ...
    // Belt-and-suspenders check — client enforces MAX_HISTORY_CHARS too.
    // Abort with an SSE error event.
...
if (isset($data['messages'])) {
    foreach ($data['messages'] as $m) {
        ...
        // Handle array-based parts from index.html
        if (is_array($m['parts'])) {
            foreach ($m['parts'] as $p) {
                if (isset($p['text'])) {
                    $parts[] = ["text" => $p['text']];
